...
namespace CG {
	...
	void InvokeExpressions(std::ofstream* stream, LEX::LEX t, int start, int end, int salt = 0)
	{
		for (int i = start; i < end; i++)
		{
			...
			//разобрать вывод WRITE
			else if (t.lextable.table[i].lexema == LEX_WRITE) {
				//pos++;
				switch (t.idtable.table[t.lextable.table[i + 1].idxTI].iddatatype)
				{
				case (IT::CHR): {
				...
				case (IT::STR): {
					...
					*stream << "CALL writeStr" << '\n';
					break;
				}
				case (IT::INT): {
					*stream << "\npush ";
					*stream << t.idtable.table[t.lextable.table[i + 1].idxTI].id << '\n';
					*stream << "CALL writeUint" << '\n';
					break;
				}
				case (IT::SHO): {
					*stream << "\npush ";
					*stream << t.idtable.table[t.lextable.table[i + 1].idxTI].id << '\n';
					*stream << "CALL writeShort" << '\n';
					break;
				}
				}
			}
			//разобрать вывод WRITELINE
			else if (t.lextable.table[i].lexema == LEX_WRITELINE) {
				//pos++;
				switch (t.idtable.table[t.lextable.table[i + 1].idxTI].iddatatype)
				{
				case (IT::CHR): {
				...
				case (IT::STR): {
					...
					*stream << "CALL writelineStr" << '\n';
					break;
				}
				case (IT::INT): {
					*stream << "\npush ";
					*stream << t.idtable.table[t.lextable.table[i + 1].idxTI].id << '\n';
					*stream << "CALL writelineUint" << '\n';
					break;
				}
				case (IT::SHO): {
					*stream << "\npush ";
					*stream << t.idtable.table[t.lextable.table[i + 1].idxTI].id << '\n';
					*stream << "CALL writelineShort" << '\n';
					break;
				}
				}
			}
			//TODO: разобрать WHILE
	...
	void Head(std::ofstream* stream, LEX::LEX t) {
		...
		*stream << "\tincludelib ../FVG-2024/Debug/FVG-2024LIB.lib\n";
		...
		*stream << "\n";
		*stream << "\n\twriteUint PROTO :DWORD";
		*stream << "\n\writeShort PROTO :WORD";
		*stream << "\n\twriteChar PROTO :BYTE";
		*stream << "\n\twriteStr PROTO :DWORD\n";
		*stream << "\n\twritelineUint PROTO :DWORD";
		*stream << "\n\twritelineShort PROTO :WORD";
		*stream << "\n\twritelineChar PROTO :BYTE";
		*stream << "\n\twritelineStr PROTO :DWORD\n";

		*stream << "\n.stack 4096\n";
	}
	void Constants(std::ofstream* stream, LEX::LEX t) {
		*stream << ".const\n";
		*stream << "num_err byte 'Error: overflow', 0\n";
		*stream << "divideOnZeroExeption BYTE \" НА НОЛЬ ДЕЛИТЬ НЕЛЬЗЯ!!!.\", 0  ;STR, вывод ошибки деления на ноль\n";
		for (int i = 0; i < t.idtable.size; i++)
		{
			if (t.idtable.table[i].idtype == IT::L)
			{
				*stream << "\t" << t.idtable.table[i].id;
				...
				case IT::SHO: {
					*stream << " SDWORD " << t.idtable.table[i].value.vshort << " ;SHO";
					break;
				}
				case IT::CHR: {
