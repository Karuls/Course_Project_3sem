...
void FST::LexAnalyzer(In::IN in, Out::OUT out, Log::LOG log, LT::LexTable& lextable, IT::IdTable& idtable)
{
	...
	FST l_numberLiteral(
		str,
		7,
		NODE(21, RELATION('1', 1), RELATION('2', 1), RELATION('3', 1), RELATION('4', 1), RELATION('5', 1), RELATION('6', 1), RELATION('7', 1), RELATION('8', 1), RELATION('9', 1),
			RELATION('1', 6), RELATION('2', 6), RELATION('3', 6), RELATION('4', 6), RELATION('5', 6), RELATION('6', 6), RELATION('7', 6), RELATION('8', 6), RELATION('9', 6),
			RELATION('0', 2), RELATION('0', 6), RELATION('-', 3)),
		NODE(20, RELATION('1', 1), RELATION('2', 1), RELATION('3', 1), RELATION('4', 1), RELATION('5', 1), RELATION('6', 1), RELATION('7', 1), RELATION('8', 1), RELATION('9', 1), RELATION('0', 1),
			RELATION('1', 6), RELATION('2', 6), RELATION('3', 6), RELATION('4', 6), RELATION('5', 6), RELATION('6', 6), RELATION('7', 6), RELATION('8', 6), RELATION('9', 6), RELATION('0', 6)),
		NODE(1, RELATION('x', 4)),
		NODE(19, RELATION('1', 1), RELATION('2', 1), RELATION('3', 1), RELATION('4', 1), RELATION('5', 1), RELATION('6', 1), RELATION('7', 1), RELATION('8', 1), RELATION('9', 1),
			RELATION('1', 6), RELATION('2', 6), RELATION('3', 6), RELATION('4', 6), RELATION('5', 6), RELATION('6', 6), RELATION('7', 6), RELATION('8', 6), RELATION('9', 6),
			RELATION('0', 2)),
		NODE(30, RELATION('1', 5), RELATION('2', 5), RELATION('3', 5), RELATION('4', 5), RELATION('5', 5), RELATION('6', 5), RELATION('7', 5), RELATION('8', 5), RELATION('9', 5), RELATION('a', 5), RELATION('b', 5), RELATION('c', 5), RELATION('d', 5), RELATION('e', 5), RELATION('f', 5),
			RELATION('1', 6), RELATION('2', 6), RELATION('3', 6), RELATION('4', 6), RELATION('5', 6), RELATION('6', 6), RELATION('7', 6), RELATION('8', 6), RELATION('9', 6), RELATION('a', 6), RELATION('b', 6), RELATION('c', 6), RELATION('d', 6), RELATION('e', 6), RELATION('f', 6)),
		NODE(32, RELATION('1', 5), RELATION('2', 5), RELATION('3', 5), RELATION('4', 5), RELATION('5', 5), RELATION('6', 5), RELATION('7', 5), RELATION('8', 5), RELATION('9', 5), RELATION('a', 5), RELATION('b', 5), RELATION('c', 5), RELATION('d', 5), RELATION('e', 5), RELATION('f', 5), RELATION('0', 5),
			RELATION('1', 6), RELATION('2', 6), RELATION('3', 6), RELATION('4', 6), RELATION('5', 6), RELATION('6', 6), RELATION('7', 6), RELATION('8', 6), RELATION('9', 6), RELATION('a', 6), RELATION('b', 6), RELATION('c', 6), RELATION('d', 6), RELATION('e', 6), RELATION('f', 6), RELATION('0', 6)),
		NODE()
	);
	...
	Checker checkArr[] = {
		...
		Checker(&l_short,LEX_CHAR,IT::SHO),
		...
		Checker(&l_write,LEX_WRITE,(IT::IDDATATYPE)FALSYNUMBER),
		Checker(&l_writeline,LEX_WRITELINE,(IT::IDDATATYPE)FALSYNUMBER),
		...
		Checker(&l_numberLiteral,LEX_LITERAL,IT::SHO),
		//len = 22
	};
	const int checkArrLen = 28;
	...
	for (int i = 0; i < sizze; i++)
	{
		...
		for (int j = 0; j < checkArrLen; j++)
		{
			if (execute(*checkArr[j].chain))
			{
				...
				//провер€ем что за лексема
				char lexema = checkArr[j].lexName;
				...
				case LEX_TYPE: {
					...
					else if (checkArr[j].iddatatype == IT::SHO)
					{
						dataType = IT::SHO;
					}
					else
				...
				case LEX_LITERAL: {
					...
					if (checkArr[j].iddatatype == IT::INT)
					{
						long out = strtoul(str, NULL, 10); // строка -> число
						if (out < 0 || out > 4294967295)
							throw ERROR_THROW(117);
						if (lextable.table[lextable.size - 1].lexema == LEX_UNARNIMINUS) {
							out *= -1;
						}
						int positionInTable = IT::IsLX(idtable, checkArr[j].iddatatype, unsigned int(out));
						if (positionInTable != TI_NULLIDX) // если нет 
						{
							continueFlag = false;
					...
					if (continueFlag)
					{
						...
						scope += "$LEX";
						char* t = new char[6];
						_itoa_s(literalCount, t, 6, 10); // число -> строка
						scope += t;
						...
						if (checkArr[j].iddatatype == IT::INT)
						{
							...
							if (strlen(str) >= 2 && str[1] == 'x') {
								out = strtol(str, NULL, 16); // строка в число
							}
							else {
								out = strtol(str, NULL, 10); // строка в число
							}
							if (lextable.table[lextable.size - 1].lexema == LEX_UNARNIMINUS) {
								if(out > 0)
								out *= -1;
								if (out >= -32768 && out <= 32767) {
									int positionInTable = IT::IsLX(idtable, IT::SHO, short(out));
									if (positionInTable != TI_NULLIDX) // если нет 
									{
										continueFlag = false;
										lexTableObject.idxTI = positionInTable;
									...
									IT::Entry ttmp(lextable.size - 1, scope.c_str(), IT::SHO, IT::L, short(out));
									IT::Add(idtable, ttmp);
				...
				if (lexTableObject.lexema == LEX_MINUS || lexTableObject.lexema == LEX_LEFTBRACE || lexTableObject.lexema == LEX_RIGHTBRACE || lexTableObject.lexema == LEX_LEFTHESIS || lexTableObject.lexema == LEX_RIGHTHESIS) {
					...
					if (lextable.table[lextable.size - 1].lexema != LEX_ID && lextable.table[lextable.size - 1].lexema != LEX_LITERAL && lexTableObject.lexema == 'v' && str[0] == '-') {

						lexTableObject.lexema = LEX_UNARNIMINUS;
					}
					else 	lexTableObject.data = str[0];
